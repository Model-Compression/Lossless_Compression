...
from activation_numpy import my_activation_numpy
...
def first_order(name, **args):
    ...
    """
    if name == 'binary_zero':
        return lambda x, s1, s2, b1, tao_last: x * my_activation_numpy(name, **args)(tao_last * x, s1, s2, b1) * stats.norm.pdf(
            x) / tao_last

    elif name == 'binary_last':
        return lambda x, s1, s2, s3, s4, b1, b2, b3, tao_last: x * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, s3, s4, b1, b2, b3) * stats.norm.pdf(x) / tao_last

    elif name == 'binary_last_final':
        return lambda x, s1, s2, b1, b2, b3, tao_last: x * my_activation_numpy(name, **args)(tao_last * x, s1, s2, b1, b2, b3
                                                                                             ) * stats.norm.pdf(x) / tao_last

    elif name == 'binary_last_four':
        return lambda x, s1, s2, s3, s4, b1, b2, tao_last: x * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, s3, s4, b1, b2) * stats.norm.pdf(x) / tao_last

    else:
        return lambda x, tao_last: x * my_activation_numpy(name, **args)(tao_last * x) * stats.norm.pdf(x) / tao_last
...
def second_order(name, **args):
    ...
    if name == 'binary_zero':
        return lambda x, s1, s2, b1, tao_last: (pow(x, 2) - 1) * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, b1) * stats.norm.pdf(x) / pow(tao_last, 2)

    # notation:!!!!!!s1 s2 location zero

    elif name == 'binary_last':
        return lambda x, s1, s2, s3, s4, b1, b2, b3, tao_last: (pow(x, 2) - 1) * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, s3, s4, b1, b2, b3) * stats.norm.pdf(x) / pow(tao_last, 2)

    elif name == 'binary_last_final':
        return lambda x, s1, s2, b1, b2, b3, tao_last: (pow(x, 2) - 1) * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, b1, b2, b3) * stats.norm.pdf(x) / pow(tao_last, 2)

    elif name == 'binary_last_four':
        return lambda x, s1, s2, s3, s4, b1, b2, tao_last: (pow(x, 2) - 1) * my_activation_numpy(name, **args)(
            tao_last * x, s1, s2, s3, s4, b1, b2) * stats.norm.pdf(x) / pow(tao_last, 2)

    else:
        return lambda x, tao_last: (pow(x, 2) - 1) * my_activation_numpy(name, **args)(tao_last * x) * stats.norm.pdf(x) / pow(
            tao_last, 2)
...
def zero_order(name, **args):
    ...
    if name == 'binary_zero':
        return lambda x, s1, s2, b1, tao_last: my_activation_numpy(name, **args)(tao_last * x, s1, s2, b1) * stats.norm.pdf(x)

    elif name == 'binary_last':
        return lambda x, s1, s2, s3, s4, b1, b2, b3, tao_last: my_activation_numpy(name, **args)(tao_last * x, s1, s2, s3, s4,
                                                                                                 b1, b2, b3) * stats.norm.pdf(x)

    elif name == 'binary_last_final':
        return lambda x, s1, s2, b1, b2, b3, tao_last: my_activation_numpy(name, **args)(tao_last * x, s1, s2, b1, b2, b3
                                                                                         ) * stats.norm.pdf(x)

    elif name == 'binary_last_four':
        return lambda x, s1, s2, s3, s4, b1, b2, tao_last: my_activation_numpy(name, **args)(tao_last * x, s1, s2, s3, s4, b1,
                                                                                             b2) * stats.norm.pdf(x)

    else:
        return lambda x, tao_last: my_activation_numpy(name, **args)(tao_last * x) * stats.norm.pdf(x)
...
def expect_calcu(name, **args):
    ...
    if name == 'binary_zero':
        zero_order_expect = lambda s1, s2, b1, tao_last: quad(
            zero_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, tao_last))[0]
        tao_expect = lambda s1, s2, b1, tao_last: quad(
            tao(name, zero_order_expect(s1, s2, b1, tao_last), **args), -np.inf, np.inf, args=(s1, s2, b1, tao_last))[0]
        first_order_expect = lambda s1, s2, b1, tao_last: quad(
            first_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, tao_last))[0]
        second_order_expect = lambda s1, s2, b1, tao_last: quad(
            second_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, tao_last))[0]
        square_second_order_expect = lambda s1, s2, b1, tao_last: quad(square_second_order(
            name, zero_order_expect(s1, s2, b1, tao_last), **args),
                                                                       -np.inf,
                                                                       np.inf,
        ...
        zero_order_expect = lambda s1, s2, s3, s4, b1, b2, b3, tao_last: quad(
            zero_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, b3, tao_last))[0]
        tao_expect = lambda s1, s2, s3, s4, b1, b2, b3, tao_last: quad(tao(
            name, zero_order_expect(s1, s2, s3, s4, b1, b2, b3, tao_last), **args),
                                                                       -np.inf,
                                                                       np.inf,
                                                                       args=(s1, s2, s3, s4, b1, b2, b3, tao_last))[0]
        first_order_expect = lambda s1, s2, s3, s4, b1, b2, b3, tao_last: quad(
            first_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, b3, tao_last))[0]
        second_order_expect = lambda s1, s2, s3, s4, b1, b2, b3, tao_last: quad(
            second_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, b3, tao_last))[0]
        square_second_order_expect = lambda s1, s2, s3, s4, b1, b2, b3, tao_last: quad(
            square_second_order(name, zero_order_expect(s1, s2, s3, s4, b1, b2, b3, tao_last), **args),
            -np.inf,
            np.inf,
        ...
        zero_order_expect = lambda s1, s2, b1, b2, b3, tao_last: quad(
            zero_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, b2, b3, tao_last))[0]
        tao_expect = lambda s1, s2, b1, b2, b3, tao_last: quad(tao(name, zero_order_expect(s1, s2, b1, b2, b3, tao_last), **args
                                                                   ),
                                                               -np.inf,
                                                               np.inf,
                                                               args=(s1, s2, b1, b2, b3, tao_last))[0]
        first_order_expect = lambda s1, s2, b1, b2, b3, tao_last: quad(
            first_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, b2, b3, tao_last))[0]
        second_order_expect = lambda s1, s2, b1, b2, b3, tao_last: quad(
            second_order(name, **args), -np.inf, np.inf, args=(s1, s2, b1, b2, b3, tao_last))[0]
        square_second_order_expect = lambda s1, s2, b1, b2, b3, tao_last: quad(square_second_order(
        ...
        zero_order_expect = lambda s1, s2, s3, s4, b1, b2, tao_last: quad(
            zero_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, tao_last))[0]
        tao_expect = lambda s1, s2, s3, s4, b1, b2, tao_last: quad(tao(name, zero_order_expect(
            s1, s2, s3, s4, b1, b2, tao_last), **args),
                                                                   -np.inf,
                                                                   np.inf,
                                                                   args=(s1, s2, s3, s4, b1, b2, tao_last))[0]
        first_order_expect = lambda s1, s2, s3, s4, b1, b2, tao_last: quad(
            first_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, tao_last))[0]
        second_order_expect = lambda s1, s2, s3, s4, b1, b2, tao_last: quad(
            second_order(name, **args), -np.inf, np.inf, args=(s1, s2, s3, s4, b1, b2, tao_last))[0]
        square_second_order_expect = lambda s1, s2, s3, s4, b1, b2, tao_last: quad(square_second_order(
            name, zero_order_expect(s1, s2, s3, s4, b1, b2, tao_last), **args),
                                                                                   -np.inf,
                                                                                   np.inf,
                                                                                   args=(s1, s2, s3, s4, b1, b2, tao_last),
                                                                                   epsrel=1.49e-10)[0]
    else:
        zero_order_expect = lambda tao_last: quad(zero_order(name, **args), -np.inf, np.inf, args=(tao_last), limit=1000)[0]
        tao_expect = lambda tao_last: quad(
            tao(name, zero_order_expect(tao_last), **args), -np.inf, np.inf, args=(tao_last), limit=1000)[0]
        first_order_expect = lambda tao_last: quad(first_order(name, **args), -np.inf, np.inf, args=(tao_last), limit=1000)[0]
        second_order_expect = lambda tao_last: quad(second_order(name, **args), -np.inf, np.inf, args=(tao_last), limit=1000)[0]
        square_second_order_expect = lambda tao_last: quad(square_second_order(name, zero_order_expect(tao_last), **args),
                                                           -np.inf,
                                                           np.inf,
                                                           limit=1000,
                                                           args=(tao_last),
...
if __name__ == "__main__":
    ...
    print(zero_order_expect(s1, s2, s3, s4, b1, b2, tao_last), first_order_expect(s1, s2, s3, s4, b1, b2, tao_last),
          second_order_expect(s1, s2, s3, s4, b1, b2, tao_last), square_second_order_expect(s1, s2, s3, s4, b1, b2, tao_last),
