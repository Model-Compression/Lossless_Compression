...
from expect_cal.expect_calculate_math import (
    custome_activation_analysis, custome_activation_analysis_noparam)
from model_define.model import My_Model
from utils.utils import calculate_CK_tilde_coef, estim_tau_tensor
            ...
            if name in ['Binary_Zero', 'Binary_Last']:
                if name == 'Binary_Zero':
                    (
                        ...
                        square_second_order,
                        tau,
                    ) = custome_activation_analysis_noparam(name='binary_zero',
                                                            **args)
                    ...
                    (
                        ...
                        square_second_order,
                        tau,
                    ) = custome_activation_analysis_noparam(name='binary_last',
                                                            **args)
...
def solve_equation(model, tau_zero, loop):
    ...
    for i in range(model.layer_num - 1):
        d1_origin, d2_origin, d3_origin = d_full[i][1], d_full[i][2], d_full[
            i][3]
        ...
        def func(x):
            return ((d1(x[0], x[1], x[2])**2 - d1_origin)**2 +
                    (d2(x[0], x[1], x[2])**2 - d2_origin)**2 +
    ...
    b1 = np.sqrt((d3_origin**2 / d2_origin) + 4 * tau_origin**2)
    b2 = b1

    def t(s1, s2, s3, s4):
        return tau(s1, s2, s3, s4, b1, b2, tau_last)
    ...
    # print(d3(-0.19, 0.623, -0.1945, 0.538))

    def func(x):
        return ((t(x[0], x[1], x[2], x[2] + x[1] - x[0]) - tau_origin**2)**2 +
                (d1(x[0], x[1], x[2], x[2] + x[1] - x[0])**2 - d1_origin)**2 +
                (d2(x[0], x[1], x[2], x[2] + x[1] - x[0])**2 - d2_origin)**2 +
                (d3(x[0], x[1], x[2], x[2] + x[1] - x[0]) - d3_origin)**2)
...
if __name__ == "__main__":
    ...
    res = solve_equation(model, tau_zero, 2000)
    activation_list = res
