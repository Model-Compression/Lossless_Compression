...
from utils.activation_tensor import my_activation_torch
...
class My_Model(nn.Module):
    ...
    def __init__(self, layer_num, input_num, weight_num_list, activation_list,
                 tau_zero) -> None:
        """Custome module with config, you can config with configuration file.
        ...
        for i in range(layer_num):
            ...
            if i == 0:
                self.fc_layers.append(
                    nn.Linear(input_num, weight_num_list[i], bias=False))
            else:
                self.fc_layers.append(
                    nn.Linear(weight_num_list[i - 1],
                              weight_num_list[i],
                              bias=False))
            ...
            if args_activ:
                if name_activ == 'Binary_Zero':
                    zero_order, _, _, _, tau_square = custome_activation_analysis_noparam(
                        'binary_zero', **args_activ)
                elif name_activ == 'Binary_Last':
                    zero_order, _, _, _, tau_square = custome_activation_analysis_noparam(
                        'binary_last', **args_activ)
                else:
                    zero_order, _, _, _, tau_square = expect_calcu(
                        name_activ, **args_activ)
            else:
